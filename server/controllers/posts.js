...
export const likePost = async (req, res) => {
  try {
    const { id } = req.params; //remember req will be attached to the event pressed
    ...
    const updatedPost = await Post.findByIdAndUpdate(
      //now update the actual Post with the const 'post' information
      id, //provide the Id
      { likes: post.likes },
      { new: true }
    );
...
export const deletePost = async (req, res, next) => {
  try {
    ...
    // const posts = await Post.find(); //grabs all the posts

    const post = await Post.findByIdAndDelete(id); //grabbing post info and deleting
    if (!post) return res.sendStatus(404);
    return res.send(post);
    // console.log(post);

    // res.status(200).json({ message: "Post Deleted" }); //update the frontend
  } catch (error) {
